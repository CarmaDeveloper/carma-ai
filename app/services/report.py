...
from app.models.report import QAItem, ReportGenerationRequest, ReportGenerationResponse, TokenUsage
from app.prompts.report import RAG_PROMPT_TEMPLATE
...
class ReportGenerationService:
    ...
    async def generate_report(
        ...
        logger.info(
            ...
            f"qa_count={len(request.qas)}, has_prompt={len(request.prompt) > 0}"
        )
    ...
    async def _generate_with_token_tracking(
        self, prompt_messages, context_docs, qa_text
        ...
        try:
            ...
            # Create a copy of prompt messages to avoid modifying the original
            chain_prompt_messages = prompt_messages.copy()

            # Add context and QA placeholders
            chain_prompt_messages.append(
                (
                    "human",
                    RAG_PROMPT_TEMPLATE,
                )
            ...
            logger.info(f"Final prompt messages: {chain_prompt_messages}")
            prompt_template = ChatPromptTemplate.from_messages(chain_prompt_messages)
            logger.info(f"Prompt template: {prompt_template}")

            # Format the prompt with context and QA data
            formatted_prompt = prompt_template.format(
                context=context_docs,  # Pass documents directly
                qas=qa_text,
            )
    ...
    def _log_bedrock_request(self, request, qa_text, context_docs):
        ...
        # Log the original prompt messages
        logger.info(f"Original prompt messages:")
        for i, (role, content) in enumerate(request.prompt):
            logger.info(f"  Message {i + 1} ({role}): {content}")
        ...
        # Log the final human message that will be sent
        final_human_message = RAG_PROMPT_TEMPLATE
        logger.info(f"Final human message template: {final_human_message}")
        ...
        try:
            ...
            if hasattr(response, 'usage_metadata') and response.usage_metadata:
                ...
                if isinstance(usage_metadata, dict):
                    ...
                    input_tokens = getattr(usage_metadata, 'input_tokens', getattr(usage_metadata, 'prompt_tokens', 0))
                    output_tokens = getattr(usage_metadata, 'output_tokens', getattr(usage_metadata, 'completion_tokens', 0))
